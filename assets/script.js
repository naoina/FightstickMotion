...
const defaultLayoutName = 'inputhistory';
...
function onLoadLayout(data) {
  ...
  document.getElementById('inputhistorytoggle').checked =
    customlayout.inputhistorymode.toggle;
  document.getElementById(
    customlayout.inputhistorymode.direction === 1
      ? 'inputhistoryhorizontal'
      : customlayout.inputhistorymode.direction === 2
        ? 'inputhistoryhorizontalup'
        : 'inputhistoryvertical'
  ).checked = true;
  document.getElementById('inputhistorycount').value =
    customlayout.inputhistorymode.count;
  document.getElementById('inputhistorygame').value =
    customlayout.inputhistorymode.game || 'default';
  ...
  for (
    let i = 0, len = (customlayout.custombutton || []).length;
    i < len;
    ++i
  ) {
    custombutton[i] = customlayout.custombutton[i];
  }
}

function loadJSON(filename) {
  var json = document.head.getElementsByClassName('json')[0];
  if (json != undefined) document.head.removeChild(json);
...
function saveLayout(filename) {
  ...
  saveToFile(
    filename,
    remark + 'onLoadLayout(' + JSON.stringify(customlayout) + ');'
  );
}
...
function getChildByClass(el, className) {
  for (var i = 0, il = el.childNodes.length; i < il; i++) {
    var classes =
      el.childNodes[i].className !== undefined
        ? el.childNodes[i].className.split(' ')
        : [];
...
function getChildById(el, id) {
  for (var i = 0, il = el.childNodes.length; i < il; i++) {
    var classes =
      el.childNodes[i].id !== undefined ? el.childNodes[i].id.split(' ') : [];
    for (var j = 0, jl = classes.length; j < jl; j++) {
      if (classes[j] == id) notes = el.childNodes[i];
    }
  }

  if (notes == el) console.log(el);
...
function setClassName(e, t) {
  if (e !== undefined && e !== null) e.className = t;
}
...
function replaceClassName(e, o, n) {
  ...
  if (regex.test(e.className)) e.className = e.className.replace(regex, n);
  else addClassName(e, n);
}

function setClassNameById(id, t) {
  var e = document.getElementById(id);
  if (e !== undefined && e !== null) setClassName(e, t);
}

function setElementValueById(id, val) {
  var e = document.getElementById(id);
  if (e !== undefined && e !== null) e.value = val;
}

function enableById(id, enable) {
  var e = document.getElementById(id);
  if (e !== undefined && e !== null) e.disabled = !enable;
}

function setValueById(id, value) {
  var e = document.getElementById(id);
  if (e !== undefined && e !== null) e.value = value;
}

function getValueById(id) {
  var e = document.getElementById(id);
  if (e !== undefined && e !== null) return e.value;
}

function addClassNameById(id, name) {
  var e = document.getElementById(id);
  if (e !== undefined && e !== null) addClassName(e, name);
}

function removeClassNameById(id, name) {
  var e = document.getElementById(id);
  if (e !== undefined && e !== null) removeClassName(e, name);
}

function toggleClassNameById(id, name, toggle) {
  var e = document.getElementById(id);
  if (e !== undefined && e !== null) {
    if (toggle) addClassName(e, name);
    else removeClassName(e, name);
  }
}
...
function disconnecthandler(e) {
  ...
  if (activegamepad !== -1) {
    ...
    if (gamepad !== undefined && gamepad !== null) {
      if (gamepad.connected) disconnected = false;
    }
  }
  ...
  if (disconnected) {
    ...
    if (polltimer === -1) polltimer = setInterval(connecthandler, 100);
  }
}
...
function showButtons(total) {
  if (typeof total !== 'undefined') {
    ...
    customlayout.totalbuttonshow =
      num < 0 ? 0 : num > totalbuttons ? totalbuttons : num;
  }
  ...
  if (Number(e.value) > e.max) e.value = e.max;
  if (e.max <= 0) {
    e.value = '';
    getButtonValue('');
    e.disabled = true;
  ...
  e.disabled = !(
    e.max > 0 && document.getElementById('custombuttoncheck').checked
  );
}

function showStickBlock(show) {
  if (typeof show !== 'undefined') customlayout.showstick = show;
  else show = customlayout.showstick;
...
function showBackground(show) {
  if (typeof show !== 'undefined') customlayout.background.show = show;
  else show = customlayout.background ? customlayout.background.show : true;
...
function resetgamepad() {
  ...
  for (var i = 0; i < 4; ++i) {
    setClassName(
      getStickArrowElement(gamepadid, i),
      'stick-block ' + sticknames[i]
    );
  }
...
function clearSetupStatus() {
  if (setbutton !== -1) {
    if (setbutton >= 1000)
      removeClassName(
        getStickSetupElement(0, setbutton - 1000),
        sticksetuplabel
      );
    else removeClassName(getButtonElement(0, setbutton), buttonsetuplabel);
...
function showElement(ele, show) {
  ...
  else replaceClassName(ele, 'hide', 'show');
}
...
function setContainerHeight() {
  document.getElementById('container').style.height =
    document.getElementById('gamepad-area').offsetHeight * options.scale + 'px';
}
...
function updateInputHistoryToggle() {
  if (document.getElementById('customlayouttoggle').checked) {
    customlayout.inputhistorymode.toggle = document.getElementById(
      'inputhistorytoggle'
    ).checked;
    document.getElementById('inputhistoryfieldset').style.display = customlayout
      .inputhistorymode.toggle
      ? 'block'
      : 'none';
    document.getElementById('stickbuttonconfig').style.display = customlayout
      .inputhistorymode.toggle
      ? 'none'
      : 'block';

    if (customlayout.inputhistorymode.toggle) {
      customlayout.inputhistorymode.direction = parseInt(
        document.querySelector("input[name='inputhistorydirectionset']:checked")
          .value
      );
      ...
      if (customlayout.inputhistorymode.direction === 0) {
        ...
        else removeClassNameById('inputlist', 'up');
      }

      document.getElementById('inputhistorytekkenfieldset').style.display =
        customlayout.inputhistorymode.game === 'tekken' ? 'block' : 'none';
...
function updateInputHistoryStyle() {
  ...
  style +=
    ".inputlistelement.inputarrow{background-image: url('layout/" +
    customlayout.name +
    "/btn-up.png');}";

  for (var i = 0; i < 20; i++) {
    style +=
      '.inputlistelement.inputlistelement' +
      i +
...
function updateInputHistoryStyleTekken(btns) {
  return (
    '.tekken .inputlistelement.inputlistelement' +
    btns.join('-') +
    "{background-image:url('layout/" +
    customlayout.name +
    '/btn-' +
...
function updateInputHistoryTekkenBtn() {
  if (customlayout.inputhistorymode.game === 'tekken') {
    var btn =
      parseInt(document.getElementById('inputhistorytekkenbtn').value) - 1;
    if (!customlayout.inputhistorymode.btnmapping)
      customlayout.inputhistorymode.btnmapping = [
        '1',
        '2',
        '3',
        '4',
    ...
    document.getElementById('inputhistorytekkenbtnmapto').value =
      customlayout.inputhistorymode.btnmapping.length > btn
        ? customlayout.inputhistorymode.btnmapping[btn]
        : '0';
  }
}
...
function updateInputHistoryTekkenBtnMapTo() {
  customlayout.inputhistorymode.btnmapping[
    document.getElementById('inputhistorytekkenbtn').value - 1
  ] = document.getElementById('inputhistorytekkenbtnmapto').value;
}

function decodeAxisCode(str, pos) {
  var val = str.substring(pos, pos + 6);
  if (val.length < 6 || val.match(/[^0-9]/) !== null) return -1;
...
function setUserSpecificButton(name) {
  ...
  if (!isNaN(button) && button > 0 && button <= totalbuttons)
    custombutton[button - 1] = axisToButtonCode(
      Number(userspecific[name].axis),
      Number(userspecific[name].value)
    );
}
...
function updateLayoutName() {
  ...
  if (customlayout.inputhistorymode.toggle) updateInputHistoryToggle();
}
...
function updateLink() {
  ...
  for (i = 0; i < 4; ++i) {
    ...
    if (customdpad[i] != i + 12) totalcustomdpads = i;
  }

  for (i = 0; i < totalbuttons; ++i) {
    if (custombutton[i] != i) totalcustombuttons = i;
  }
  for (i = 0; i <= totalcustombuttons; ++i) {
    btnstr += encodeButtonCode(custombutton[i]);
  }

  if (totalcustombuttons > 0) para += '&b=' + dpadstr + btnstr;
  else if (totalcustomdpads > 0) para += '&b=' + dpadstr;
  ...
  if (document.getElementById('customlayouttoggle').checked) {
    ...
    customlayout.custombutton = custombutton;
    var pathname = 'layout/' + name + '/';
    document.getElementById(
      'inputfilenamepathbackgroundimg'
    ).innerHTML = pathname;
    ...
    document.getElementById('tooltiptextsavefile').innerHTML =
      'Copy the saved file to the "' + pathname + '" folder';
    document.getElementById('tooltiptextimagefile').innerHTML =
      'Image files must be placed under the "' + pathname + '" folder';
    document.getElementById('tooltiptextimagepfile').innerHTML =
      'Image files must be placed under the "' + pathname + '" folder';

    if (name !== '') para += '&l=' + encodeURI(name);
...
function checkUserSpecificStickTilted(obj, gamepad) {
  if (obj.axis === '' || obj.value === '') return -1;

  var axis = Number(obj.axis);
  var value = Number(obj.value);

  if (axis >= gamepad.axes.length) return -1;

  var gamepadaxis = gamepad.axes[axis];

  if (value >= gamepadaxis - 0.05 && value <= gamepadaxis + 0.05) return 1;
...
function checkUserSpecificStick(obj, neutral_obj, gamepad) {
  ...
  var neutral =
    (value - (neutral_obj.value === '' ? 0 : Number(neutral_obj.value))) * 0.3;
  ...
  if (max >= gamepadaxis - 0.05 && min <= gamepadaxis + 0.05) return 1;
    ...
    if (axis < gamepad.axes.length) {
      if (userspecific.enable) {
        for (var i = 0; i < userspecific.status.length; ++i) {
          ...
          if (
            axis == userspecific.status[i].axis &&
            diff <= 0.05 &&
            diff >= -0.05
          )
            return 1;
      ...
      if (gamepadaxis >= min && gamepadaxis <= max) return 1;
...
var setaxiscounter = {
  ...
  set: function(axis, value) {
    ...
    this.value = value;
    this.counter = 1;
    addClassNameById('assignbuttontooltip', 'tooltiptextholdaxis');
  },
  clear: function() {
    this.axis = 0;
    this.value = 0;
  ...
  if (activegamepad !== -1) {
    ...
    if (setbutton !== -1) {
      for (i = 0; i < btnlen; ++i) {
        if (gamepad.buttons[i].pressed) {
          if (setbutton < 1000) custombutton[setbutton] = i;
          else customdpad[setbutton - 1000] = i;
        ...
        for (i = 0; i < axeslen; ++i) {
          ...
          if (
            ...
            if (setaxiscounter.axis === i) {
              if (
                setaxiscounter.value <= val + 0.3 &&
                setaxiscounter.value >= val - 0.3
              ) {
                if (++setaxiscounter.counter >= 60) {
                  var code = axisToButtonCode(i, val);

                  if (setbutton < 1000) custombutton[setbutton] = code;
                  else customdpad[setbutton - 1000] = code;
            ...
            if (reset) setaxiscounter.set(i, val);
          }
        }

        if (axiscenter && setaxiscounter.counter > 0) setaxiscounter.clear();
      }
    }
    ...
    if (userspecific.enable) {
      ...
      if (tilted) {
        if (
          status[0] === 0 &&
          checkUserSpecificStickTilted(userspecific.u, gamepad) > 0
        ) {
        ...
        if (
          status[1] === 0 &&
          checkUserSpecificStickTilted(userspecific.d, gamepad) > 0
        ) {
        ...
        if (
          status[2] === 0 &&
          checkUserSpecificStickTilted(userspecific.l, gamepad) > 0
        ) {
        ...
        if (
          status[3] === 0 &&
          checkUserSpecificStickTilted(userspecific.r, gamepad) > 0
        ) {
        ...
        if (
          checkUserSpecificStick(userspecific.u, userspecific.n, gamepad) > 0
        ) {
          stickclass += ' ' + sticknames[0];
          status[0] = 1;
          userspecific.status.push(userspecific.u);
        ...
        if (
          checkUserSpecificStick(userspecific.d, userspecific.n, gamepad) > 0
        ) {
          stickclass += ' ' + sticknames[1];
          status[1] = 1;
          userspecific.status.push(userspecific.d);
        ...
        if (
          checkUserSpecificStick(userspecific.l, userspecific.n, gamepad) > 0
        ) {
          stickclass += ' ' + sticknames[2];
          status[2] = 1;
          userspecific.status.push(userspecific.l);
        ...
        if (
          checkUserSpecificStick(userspecific.r, userspecific.n, gamepad) > 0
        ) {
          stickclass += ' ' + sticknames[3];
          status[3] = 1;
          userspecific.status.push(userspecific.r);
...
*/
    ...
    // buttons
    var istekken =
      activelayout.inputhistorymode.toggle &&
      activelayout.inputhistorymode.game === 'tekken';
    ...
    for (i = 0; i < activelayout.totalbuttonshow; i++) {
      ...
      if (btnstatus !== buttonstatus[i]) {
        ...
        if (btnstatus === 1) {
          ...
          if (istekken) statuschanged = true;
        }
      }
    }
    ...
    if (activelayout.inputhistorymode.toggle) {
      ...
      var d = 0;
      if (statuschanged) {
        if (dpadstatus[0] === 1 && dpadstatus[2] === 1) d = 1005;
        else if (dpadstatus[1] === 1 && dpadstatus[2] === 1) d = 1006;
        else if (dpadstatus[0] === 1 && dpadstatus[3] === 1) d = 1007;
        else if (dpadstatus[1] === 1 && dpadstatus[3] === 1) d = 1008;
        else if (dpadstatus[0] === 1) d = 1001;
        else if (dpadstatus[1] === 1) d = 1002;
        else if (dpadstatus[2] === 1) d = 1003;
        else if (dpadstatus[3] === 1) d = 1004;
...
function displayInputListTekken(input) {
  ...
  for (var i = 0; i < len; i++) {
    inp = input[i];
    if (inp >= 1000 && inp < 2000) arrow = inp;
  }
  ...
  for (i = 0; i < activelayout.totalbuttonshow; i++) {
    if (buttonstatus[i] === 1) {
      ...
      if (
        activelayout.inputhistorymode.btnmapping.length > i &&
        activelayout.inputhistorymode.btnmapping[i] !== '0'
      ) {
  ...
  } else removeClassName(ele, 'pressing');
...
function enableStyle(id, enable) {
  ...
  if (style !== null) style.disabled = !enable;
}
...
function removeAllButtonStyle() {
  var style = document.getElementById('button-style');
  if (style !== null) document.head.removeChild(style);
}
...
function checkInputNumber2(e) {
  ...
  else checkInputNumber(e);
}

function formatInputNumber2(e) {
  var x = e.value;
  if (e.beforevalue === undefined) e.beforevalue = e.defaultValue;
...
function formatInputNumber(e) {
  ...
  if (!e.validity.valid) e.value = e.beforevalue;
  else if (e.value != '') {
...
function getBackgroundValue() {
  ...
  document.getElementById('custombackgroundshow').checked =
    customlayout.background.show;
...
function getButtonValue(btn) {
  ...
  if (oldsetbuttonvalue !== btn) {
    ...
    if (btn >= 0) addClassNameById('button' + btn, 'setbuttonvalue');
...
function cssLoaded() {
  ...
  if (val !== '') customlayout.totalbuttonshow = Number(val);

  val = getCustomCSSValue(vals, 'showstick');
  if (val !== '') customlayout.showstick = val === 'true';
...
function saveCSSToFile() {
  ...
  data += '\\*-------------------------------------*/\r\n\r\n';
  data +=
    '#customLayoutValue\r\n{\r\n\tcontent = "totalbuttonshow=' +
    customlayout.totalbuttonshow +
    ';showstick=' +
    (customlayout.showstick ? 'true' : 'false') +
    '";\r\n}\r\n\r\n';
...
function mousedown(e) {
  if (mouseevent.target !== null) return;

  clearSetupStatus();

  if (e.target.id.match(/^button/)) {
    mouseevent.id = Number(e.target.id.match(/\d+$/));
    if (mouseevent.id >= customlayout.totalbuttonshow) return;
  } else if (
    e.target.id.match(/stick-area/) ||
    e.target.parentNode.id.match(/stick-area/)
  ) {
    mouseevent.id = 1000;
  } else return;
...
function mousemove(e) {
  ...
  var x = Math.round(
    (e.pageX - mouseevent.pagex) / options.scale + mouseevent.offsetx
  );
  var y = Math.round(
    (e.pageY - mouseevent.pagey) / options.scale + mouseevent.offsety
  );

  if (mouseevent.id < 1000) {
    // button
    if (
      document.getElementById('custombuttonn').valueAsNumber ===
      mouseevent.id + 1
    ) {
    ...
    enableSetupTooltip(
      'Move Button ' + (mouseevent.id + 1) + '<br>(' + x + ',' + y + ')'
    );
  } else if (mouseevent.id === 1000) {
    // stick
...
function mouseup(e) {
  ...
  if (document.getElementById('customlayouttoggle').checked) {
    ...
    if (mouseevent.target === e.target) clicked = true;
  }

  if (clicked) {
    if (mouseevent.id < 1000) buttonSetup(mouseevent);
    else if (mouseevent.id === 1000) {
...
function copyLayout(layout) {
  ...
  this.inputhistorymode = new inputhistorymode(
    defaultArg(layout.inputhistorymode, {})
  );
  ...
  if (layout.background) this.background = layout.background;
...
function inputhistorymode(input) {
  ...
  this.btnmapping = defaultArg(input.btnmapping, [
    '1',
    '2',
    '3',
    '4',
...
function sticklayout(layout) {
  ...
  this.x = defaultArg(layout.x, '0');
  this.y = defaultArg(layout.y, '0');
  this.w = defaultArg(layout.w, '');
  this.h = defaultArg(layout.h, '');
  return this;
}
...
function buttonlayout(layout) {
  ...
  this.w = defaultArg(layout.w, '');
  this.h = defaultArg(layout.h, '');
  this.img = defaultArg(layout.img, '');
  this.xp = defaultArg(layout.xp, '');
  this.yp = defaultArg(layout.yp, '');
  this.wp = defaultArg(layout.wp, '');
  this.hp = defaultArg(layout.hp, '');
  this.imgp = defaultArg(layout.imgp, '');
  return this;
}
...
function defaultLayout() {
  ...
  this.defaultbuttons = new buttonlayout({
    x: '',
    y: '',
    w: '60',
    h: '60',
...
function init() {
  ...
  if (player !== '') player = parseInt(player);
  else player = 0;
  ...
  var layout = getParameterByName('l') || defaultLayoutName;
...
*/
  ...
  if (customize >= 1) {
    ...
    if (location.protocol === 'file:') showElementById('obsbrowserlink');
